...
void * create_user_directory()
{
	...
	//refer to the project documentation for the detailed steps
	//cprintf("hello from the other function :v");

	//allocate a new page for the user directory
	uint32* userDirectory = kmalloc(PAGE_SIZE);

	//copy the kernel directory into the user directory
//	int i;
//	for(i = 0; i < 1024; i++)
//		userDirectory[i] = ptr_page_directory[i];

	//change this "return" according to your answer
	return (void*) userDirectory;
}
...
//my helper functions
void freeDir(struct Env* e){
	//first get the page dir. physical address
	uint32 pageDirPA = kheap_physical_address((uint32) e->env_page_directory);

	//get the frame info of this page file
	struct Frame_Info* frameInfo = to_frame_info(pageDirPA);

	//set the frame info reference to zero and free it
...
void env_free(struct Env *e)
{
	...
	//free the ws pages in the main memory
	int entryIndex;
	for(entryIndex = 0; entryIndex < e->page_WS_max_size; entryIndex++)
		unmap_frame(e->env_page_directory, (void*) e->ptr_pageWorkingSet[entryIndex].virtual_address);
	//free the working set space from the kernel heap
	kfree(e->ptr_pageWorkingSet);

	//free all page tables from the main memory
	int pageTableIndex = 0;
	while (pageTableIndex < PDX(USER_TOP)) {
		//check if the page table exist
		if (e->env_page_directory[pageTableIndex] != 0) {
			//remove it
			uint32 pageTablePA = (e->env_page_directory[pageTableIndex] >> 12)
					* PAGE_SIZE;
			//get the page table virtual address then remove it
			uint32 pageTableVA = kheap_virtual_address(pageTablePA);
			kfree((void*) pageTableVA);
		}
		pageTableIndex++;
	}

	/*uint32 va;
	for(va = 0; va < USTACKTOP; va += PAGE_SIZE * 1024){
		//free page table for each 4MB
		//first get the virtual address of that page table
		uint32* pageTableVA = NULL;
		get_page_table(e->env_page_directory, (void*) va, &pageTableVA);
		if (pageTableVA == NULL)
			return;
	...
	/*
	uint32 i;
		uint32 virt_to_remove;
			for(i=0;i<e->page_WS_max_size;i++)
			{
				virt_to_remove=env_page_ws_get_virtual_address(e, i);
				unmap_frame(e->env_page_directory,(void*)virt_to_remove);
	  ...
	  uint32 * table;
	  i=0;
	  while (i<PDX(USER_TOP))
	  			{
	  				if(e->env_page_directory[i] != 0)
	  				{
	  					uint32 ph=(e->env_page_directory[i]>>12)*PAGE_SIZE;
	  					uint32 va=kheap_virtual_address(ph);
	  					kfree((void *)va);
	  				}
	  				i++;
	  			}
